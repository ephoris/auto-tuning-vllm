...
class MLPerfBenchmark(BenchmarkProvider):
    ...
    def _build_mlperf_command(
        ...
        return [
            "python3",
            "SUT_VLLM_SingleReplica_Server.py",
            "--model-name",
            config.model,
            "--dataset-path",
            config.dataset,
    ...
    def _parse_mlperf_results(self, results_dir: str) -> Dict[str, Any]:
        ...
        for root, dirs, files in os.walk(results_dir):
            for file in files:
                if file.endswith('.txt') or 'summary' in file.lower() or 'result' in file.lower():
            ...
            try:
                ...
                for line in content.split('\n'):
                    ...
                    # Mean latency (convert from nanoseconds to milliseconds)
                        ...
                        metrics['latency_mean'] = mean_latency_ns / 1_000_000  # Convert to ms
                    ...
                    # 99th percentile latency (convert from nanoseconds to milliseconds)
                        ...
                        metrics['latency_p99'] = p99_latency_ns / 1_000_000  # Convert to ms

                # Return metrics if we found the required ones
                if all(key in metrics for key in ['throughput', 'tokens_per_second', 'latency_mean', 'latency_p99']):
                    return metrics
